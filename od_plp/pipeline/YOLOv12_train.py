...
def parse_args():
    """
    Parse command line arguments for YOLOv12 training
    """
    parser = argparse.ArgumentParser(description='Train YOLOv12 model for mammography object detection')
    ...
    parser.add_argument('--model', type=str, default='yolo12m.pt', help='Model to train, e.g. yolov8n.pt, yolov8s.pt, etc.')
    parser.add_argument('--project', type=str, default='runs/detect', help='Project name')
    parser.add_argument('--name', type=str, default=None, help='Experiment name (default: YOLOv12_timestamp)')
    ...
    parser.add_argument('--erasing', type=float, default=0.3, help='Random erasing augmentation probability')

    
    return parser.parse_args()


def train_yolov12(args):
    """
    Train a YOLOv12 model with the specified parameters
    """
    # Load a model
    ...
    if args.name is None:
        args.name = f"YOLOv12_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    print(f"Training YOLOv12 model with the following parameters:")
...
def main():
    """
    Main function to run YOLOv12 training
    """
    # Parse command line arguments
    args = parse_args()
    
    # Train the model
    model, results = train_yolov12(args)
